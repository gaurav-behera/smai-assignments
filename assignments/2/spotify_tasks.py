...
def save_reduced_dataset(n_components):
    ...
    df = df[cols]
    df.to_csv(
        os.path.join(base_dir, "data", "processed", f"spotify-reduced.csv")
    )
    
def sample_knn_test(k, metric):
    data = pd.read_csv(os.path.join(base_dir, "data", "processed", "spotify-reduced.csv"), index_col=0)
...
def sample_knn(k ,metric, reduced=False):
    ...
    print("KNN Model with k =", k, "and metric =", metric, f"{'on reduced data' if reduced else ''}")
    ...
    result = {'accuracy': metrics.accuracy(y_test, y_pred), 'inference_time': end-start}
